...
def create_map(width, height, clearance):
    ...
    # cv2.rectangle(obstacle_map, (100 - clearance, 100 - clearance), (300 + clearance, 200 + clearance), (0, 0, 0), -1)


    cv2.rectangle(obstacle_map, (1500, 0), (1750, 1000), (0,0,0), -1)
    

    cv2.rectangle(obstacle_map, (2500, 1000), (2750, 2000), (0,0,0), -1)
    

    cv2.circle(obstacle_map, (4200,800),600,(0,0,0),-1)
...
if __name__ == "__main__":
    ...
    goal = (5750, 1000, 0)  # Goal position (x, y, not considering theta for goal)
    obstacle_map = create_map(map_width, map_height, total_clearance_mm)  # Create the obstacle map
    path = a_star(start, goal, actions, obstacle_map, visualization=True)

    if path:
        draw_path(path, obstacle_map)
    else:
        print("No path found.")
