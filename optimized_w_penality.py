...
robot_diameter_mm = 400  # Approximation to the larger dimension in mm for clearance calculation
...
def get_distance_to_nearest_obstacle(node, obstacle_map):
    """
    Calculates the minimum distance from a node to the nearest obstacle on the map.
    
    Parameters:
    - node: A tuple (x, y, theta) representing the node's position and orientation.
    - obstacle_map: A numpy array representing the map, where obstacles are marked in a specific color.
    
    Returns:
    - The minimum distance from the node to the nearest obstacle.
    """
    y, x, _ = node  # Only use x and y for distance calculation, ignore theta
    obstacle_indices = np.where(np.all(obstacle_map[:, :, :3] != [255, 255, 255], axis=-1))  # Checking all three channels for non-white pixels
    if obstacle_indices[0].size == 0:
        return float('inf')  # Return infinity if there are no obstacles detected
    distances = np.sqrt((x - obstacle_indices[1])**2 + (y - obstacle_indices[0])**2)
    return np.min(distances)


def calculate_proximity_cost(node, obstacle_map):
    # This is a placeholder for the actual implementation
    # Ideally, this function returns a higher cost for nodes closer to obstacles
    proximity = get_distance_to_nearest_obstacle(node, obstacle_map)
    cost = max(0, (1 - proximity / desired_clearance))  # Normalize and invert distance
    return cost * penalty_weight  # Adjust penalty_weight based on how aggressively you want to avoid obstacles

def heuristic(node, goal, obstacle_map):
    # Basic Euclidean distance
    base_cost = np.sqrt((node[0] - goal[0])**2 + (node[1] - goal[1])**2)
    # Add an additional cost based on proximity to the nearest obstacle
    proximity_cost = calculate_proximity_cost(node, obstacle_map)
...
def a_star(start, goal, actions, base_obstacle_map, visualization=True):
    open_set = []
    heapq.heappush(open_set, (heuristic(start, goal, base_obstacle_map), start))
    came_from = {}
    g_score = {start: 0}
    ...
    while open_set:
        ...
        for action in actions:
            ...
            if tentative_g_score < g_score.get(neighbor, float('inf')):
                ...
                f_score = tentative_g_score + heuristic(neighbor, goal, base_obstacle_map) * HEURISTIC_WEIGHT
...
def main(args=None):
    ...
    goal = (5750, 700, 0)  # Goal position, ensure to have a consistent format with start
    # start = (100,1900, 0)
    # goal = (5000, 1500, 0)
