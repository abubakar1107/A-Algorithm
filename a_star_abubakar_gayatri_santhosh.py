...
def a_star(start, goal, grid, grid_visual, L=100):
    open_set = []
    # Heuristic is now based on x, y only
    ...
    while open_set:
        ...
        if heuristic(current[:2], goal[:2]) <= 5 and abs(current[2] - goal[2]) <= 5:
            path = []
        ...
        for neighbor in get_neighbors(current, L):
            x, y, theta = neighbor
            # Check boundaries and if it's an obstacle
...
try:
    ...
    path, grid_visual = a_star(start, goal, grid_binary, grid_visual)
    if path:
        print("Path found:", path)
        cv2.imshow("Explored Nodes and Path", grid_visual)
        cv2.waitKey(0)
