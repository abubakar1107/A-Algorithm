...
def calculate_new_position(x, y, theta, UL, UR, dt=0.01):
    t= 0
...
def is_trajectory_collision_free(xi, yi, thetai, UL, UR, obstacle_map, dt=0.01):
    """
...
def rpm_to_velocity(actions, R, L):
    ...
    for action in actions:
        ...
        linear_x = (V_r + V_l) / 2
        angular_z = (V_r - V_l) / L
        converted_actions.append([linear_x/1000, -angular_z])
...
class TurtlebotPathPlanner(Node):
    def __init__(self, node_name='turtlebot_path_planner'):
        super().__init__(node_name)
        self.linear_pid = PIDController(kp=0.1001, ki=0.0, kd=0.0)
        self.angular_pid = PIDController(kp=0.1001, ki=0.0, kd=0.0)
        self.actual_linear_velocity = 0.0
        self.actual_angular_velocity = 0.0
        self.publisher_ = self.create_publisher(Twist, 'cmd_vel', 10)  # Create publisher for Twist messages
        self.index = 0  # Initialize index attribute

    def execute_path(self, path, rpm_actions):
        velocities = rpm_to_velocity(rpm_actions, R, L)
        self.set_desired_velocities(velocities)

        obstacle_map = np.array(create_map(map_width, map_height, total_clearance_mm))  # Convert to numpy array

        # Using a separate timer to call publish_velocity
        timer_period = 1.0  # seconds (adjust as needed)
        self.create_timer(timer_period, self.publish_velocity)

        try:
            rclpy.spin(self)  # Blocks execution, handling callbacks
    ...
    def publish_velocity(self):
        ...
        if self.index < len(self.desired_velocities):
            desired_linear, desired_angular = self.desired_velocities[self.index]
            print("Desired Linear Velocity:", desired_linear)
            print("Desired Angular Velocity:", desired_angular)
            linear_correction = self.linear_pid.compute(desired_linear, self.actual_linear_velocity)
            angular_correction = self.angular_pid.compute(desired_angular, self.actual_angular_velocity)

            final_linear_vel = desired_linear + linear_correction
            final_angular_vel = desired_angular + angular_correction

            vel_msg = Twist()
            vel_msg.linear.x = final_linear_vel
            vel_msg.angular.z = final_angular_vel
            
            self.publisher_.publish(vel_msg)

            self.get_logger().info(f'Index: {self.index}, Publishing: Linear = {final_linear_vel:.2f}, Angular = {final_angular_vel:.2f}')
            self.get_logger().info(f'PID Correction: Linear = {linear_correction:.2f}, Angular = {angular_correction:.2f}')
            self.get_logger().info(f'Actual Velocities: Linear = {self.actual_linear_velocity:.2f}, Angular = {self.actual_angular_velocity:.2f}')
            
            self.index += 1
        else:
            self.index = 0  # Optionally reset index to loop through velocities again

    def odom_callback(self, msg):
        self.actual_linear_velocity = msg.twist.twist.linear.x
        self.actual_angular_velocity = msg.twist.twist.angular.z

    def set_desired_velocities(self, desired_velocities):
        """
        Set the desired velocities to be published.

        Parameters:
        - desired_velocities: List of tuples containing linear and angular velocities.
        """
        self.desired_velocities = desired_velocities
        self.index = 0  # Reset index when setting new velocities
...
# #     obstacle_map = np.array(create_map(self.map_width, self.map_height, self.total_clearance_mm))  # Convert to numpy array
...
# #             # Convert RPM actions to linear and angular velocities
# #             linear_x, angular_z = rpm_to_velocity([action], R, L)[0]  # Assuming rpm_to_velocity returns a list of [linear_x, angular_z]
# #             velocities.append((linear_x,-angular_z))
...
#         self.index = 0  # Reset index when setting new velocities
    ...
    #         # Append the final velocities to desired_velocities
    #         self.desired_velocities[self.index] = (final_linear_vel, final_angular_vel)
    #         print(self.desired_velocities[self.index])
    ...
    #         self.get_logger().info(f'Actual Velocities: Linear = {self.actual_linear_velocity:.2f}, Angular = {self.actual_angular_velocity:.2f}')
            ...
            #     .loginfo("ul:{}, ur:{}, vx:{}, rz:{}".format(ul[i],ur[i], vx[i],cmd.angular.z))
            #     start = time.time()
            #     counter=0      
...
def main(args=None):
    ...
    # Initialize the TurtlebotPathPlanner node with a node name
    node = TurtlebotPathPlanner(node_name='turtlebot_path_planner')

    path, rpm_actions = a_star(start, goal, actions, obstacle_map, visualization=True)

    if path and rpm_actions:
        node.execute_path(path, rpm_actions)
    else:
