...
class TurtleBotPathFollower(Node):
    ...
    def follow_path(self):
        for u_l, u_r in self.action_sets:
            vel_msg = Twist()
            
            # Convert RPM to rad/s for ROS (1 RPM = 2*pi/60 rad/s)
            u_l_rad_s = u_l * (2 * 3.14159 / 60)
            u_r_rad_s = u_r * (2 * 3.14159 / 60)

            # Assuming the left and right wheel velocities are provided in RPM
            # and your robot's wheel radius and the distance between wheels (wheel_base) are known
            wheel_radius = 0.1  # Example: 10 cm radius, adjust according to your robot
            robot_wheel_base = 0.5  # Example: 50 cm, adjust according to your robot
            
            # Convert wheel speeds to linear and angular velocities
            vel_msg.linear.x = wheel_radius * (u_l_rad_s + u_r_rad_s) / 2
            vel_msg.angular.z = wheel_radius * (u_r_rad_s - u_l_rad_s) / robot_wheel_base

            # Publish the velocity command
            self.publisher_.publish(vel_msg)
            
            # Sleep to give time for the robot to execute the command
            time.sleep(1)# Adjust sleep time as needed for your specific case
...
def main(args=None):
    ...
    turtlebot_path_follower = TurtleBotPathFollower( rpm_actions)
    rclpy.spin(turtlebot_path_follower)

    turtlebot_path_follower.destroy_node()
    rclpy.shutdown()
